...
import { rateLimit, sanitizeFormInput } from '../../lib/security';
...
async function fetchUrl(url: string): Promise<CrawlResult> {
  try {
    ...
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'SEO Shouts Anchor Cloud Analyzer/1.0 (+https://seoshouts.com)',
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: 'Invalid request format. Please ensure you are sending valid JSON.' },
        { status: 400 }
      );
    }

    const { url: inputUrl, recaptchaToken, sitemapUrl, manualUrls, step } = body;
